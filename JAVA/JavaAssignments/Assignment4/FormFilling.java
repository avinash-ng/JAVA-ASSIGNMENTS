...
{
    public static String findDates(String first, String current) throws ParseException
    {
        SimpleDateFormat obj = new SimpleDateFormat("dd-MM-yyyy");
        Calendar c_obj = Calendar.getInstance();
        c_obj.setTime(obj.parse(first));
        int val = Calendar.DAY_OF_MONTH;
        
        //finding dates + or - 30 days of filling the form
        c_obj.add(val, 31);
        String endDate = obj.format(c_obj.getTime());
        c_obj.add(val, -61);
        String startDate = obj.format(c_obj.getTime());
        
        String newStartDate = startDate.substring(0, 6)+current.substring(6, 10);
        String newEndDate = endDate.substring(0, 6)+current.substring(6,10);
        
        //converting into dates for easy comparision
        Date d1_start = obj.parse(newStartDate);
        Date d2_end = obj.parse(newEndDate);
        Date d3_current = obj.parse(current);
        
        //case when first filling date falls in the month of december
        if(d1_start.compareTo(d2_end) > 0)
        {
            d2_end = obj.parse(newEndDate.substring(0,6)+(Integer.parseInt(newEndDate.substring(6,10))+1));
        }
        
        //finding the required result
        if((d3_current.compareTo(d1_start) >0) && (d3_current.compareTo(d2_end) < 0)){
            return obj.format(d1_start)+" "+obj.format(d3_current);
        }
        else if((d3_current.compareTo(d1_start) >0) && (d3_current.compareTo(d2_end) > 0))
            return obj.format(d1_start)+" "+obj.format(d2_end);
        else
            return "No range";
    }
